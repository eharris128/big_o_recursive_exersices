...
// For each iteration through this function there are two runtimes through the function. This will produce
// an exponential runtime. O(2^n)
// let counter = 0;
...
// This will be linear relationship as the number of iterations through the loop directly relates to the size of num. O(n) 
function countSheep(num, i=0){
  i++;
  ...
  //this will be executed until it reaches base case
    ...
    // console.log(`I have run ${num-i} times`)
...
// Linear. O(n). Number of runs directly relates to the length of the array.
...
// Linear. O(n). Number of runs directly relates to the length of the string.
...
// Linear. O(n). Number of runs directly relates to the distance between n and 2.
function triangle(n, i=0) {
  i++;
  // console.log('test' + i)
  if (n < 2) 
    return n;
  return n + triangle(n - 1, i);
}
triangle(10);
/*=================================================================================
...
// Linear - O(n) - you much go thourgh every single item in the str to determine where the separators are
...
// This problem is in the solution set twice with slightly different solutions?d
// For each iteration through this function there are two runtimes through the function. This will produce
// an exponential runtime. O(2^n)
...
// O(n!) - factorial time. Factorial because as you add on each letter, you are adding an additional letter that
// has to be placed at each position throughout the word.
...
// Longer array: contains .filter and .forEach: therefore I would estimate that it is O(n^2) - quadratic
// ==============================
...
// The number of runs through this function is directly related to the distance between n and 0. Therefore it will be O(n) - linear. 
...
// Each run through the fib function results in two more calls to the function
// Therefore it must be O(2^n)
...
// Quadratic due to the nested loop. O(n^2)
var organization = {
  'Zuckerberg': {		
    'Schroepfer': {
      'Bosworth': {
